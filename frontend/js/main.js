// Handle login type selection
function handleLogin(type) {
    let redirectUrl = '';
    let message = '';

    switch (type) {
        case 'teacher':
            message = 'Redirecting to Teacher Dashboard...';
            redirectUrl = './teacher-login.html';
            break;
        case 'admin':
            message = 'Redirecting to Administration Panel...';
            redirectUrl = './admin-login.html';
            break;
        case 'government':
            message = 'Redirecting to Government Portal...';
            redirectUrl = './gov-login.html';
            break;
    }

    // Close modal
    const modal = bootstrap.Modal.getInstance(document.getElementById('loginModal'));
    modal.hide();


    // In a real application, you would redirect to the appropriate login page
    window.location.href = redirectUrl; // This line redirects the user
}

// Add smooth scrolling for anchor links
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
        e.preventDefault();
        const target = document.querySelector(this.getAttribute('href'));
        if (target) {
            target.scrollIntoView({
                behavior: 'smooth',
                block: 'start'
            });
        }
    });
});


// Add animation on scroll
const observerOptions = {
    threshold: 0.1,
    rootMargin: '0px 0px -50px 0px'
};

const observer = new IntersectionObserver(function (entries) {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            entry.target.style.opacity = '1';
            entry.target.style.transform = 'translateY(0)';
        }
    });
}, observerOptions);

// Observe all feature cards
document.querySelectorAll('.feature-card').forEach(card => {
    card.style.opacity = '0';
    card.style.transform = 'translateY(30px)';
    card.style.transition = 'opacity 0.6s ease, transform 0.6s ease';
    observer.observe(card);
});